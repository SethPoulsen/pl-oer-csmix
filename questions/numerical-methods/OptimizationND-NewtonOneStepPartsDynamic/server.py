...
import numpy as np
from sympy import var, sin, cos, exp, diff, latex
...
def generate(data):
    ...
    def func():
        x, y = var('x y')
        f = coeffs[0]*x**2 + coeffs[0]*y**2 + coeffs[1]*x*y + coeffs[2]*exp(coeffs[3]*x*y) + coeffs[4]*sin(y)**2 + coeffs[5]*cos(x*y)
        return f, x, y
    ...
    def gradient(x,y):
        ...
        return np.array([g1,g2]).astype(np.float64)
