...
if __name__ == "__main__":
  ...
  try:
    ...
    record_failure_and_exit("Error in solution query[1]: " + str(e))
  ...
  try:
    # args=['tswift2']
    solution_cursor.callproc('Result2')
    for solution_row in solution_cursor.stored_results():
      solution_rows.append(solution_row.fetchall())
    if is_modification:
      solution_rows.clear()
    ...
    record_failure_and_exit("Error in solution query[2]: " + str(e2))
  ...
  try:
    ...
    student_cursor.callproc('Result')
    for student_row in student_cursor.stored_results():
      student_rows.append(student_row.fetchall())
    if is_modification:
      student_rows.clear()
