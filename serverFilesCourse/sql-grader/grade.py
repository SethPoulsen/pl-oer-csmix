...
if __name__ == "__main__":
    ...
    solution_rows = []
    solution_rows_returned = []
    # print(os.system("pwd"), "/grade/tests/solution.sql")
    try:
        ...
        setup_sol_queries = sqlparse.split(solution_query)
        for query in setup_sol_queries:
            solution_cursor.execute(query)

        # Even if this is a query that will be checked by a dump,
        # we still need to clear the cursor

        if solution_cursor.description is not None:
            solution_rows_returned.append(tuple([x[0] for x in solution_cursor.description]))
            solution_rows_returned.append("- - - -" for x in solution_cursor.description)

        for solution_row in solution_cursor:
            solution_rows.append(solution_row)
            solution_rows_returned.append(solution_row)
        if is_modification:
            solution_rows.clear()
    ...
    try:
        ...
        setup_std_queries = sqlparse.split(student_query)

        for query in setup_std_queries:
            student_cursor.execute(query)

        if student_cursor.description is not None:
            student_rows_returned.append(tuple([x[0] for x in student_cursor.description]))
            student_rows_returned.append("- - - -" for x in student_cursor.description)

        for student_row in student_cursor:
            student_rows.append(student_row)
            student_rows_returned.append(student_row)
    ...
    output += "================\n"
    output += list_of_tuples_to_string(solution_rows_returned)
    output += "\n\n"
    output += "Actual results\n"
    output += "==============\n"
    output += list_of_tuples_to_string(student_rows_returned)

    grading_result = {}
    grading_result['score'] = (1 if success else 0.0)
    grading_result['succeeded'] = success
    grading_result['message']= ("Great job!" if success else "")
