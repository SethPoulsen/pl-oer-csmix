...
class DataConstraintGenerator:
    ...
    def op_case(self, val):
        """
            allows the code to pass in specific cases that can be alternated in the dataset_generator
            i.e. Enrollments, Credits, case, 1
            this will trigger the "specal case 1" that is hard-coded in the generator
            this is considered a hot fix and the data constraint generator should be rewritten.
            Please list all cases used here when adding a new case: 1;
        """
        val = ['case'+str(v) for v in val]
        return self.op_in(val)
