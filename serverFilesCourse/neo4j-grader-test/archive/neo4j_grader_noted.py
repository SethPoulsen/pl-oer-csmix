...
class Neo4jGrader:
	...
	def grade(self):
		...
		print("[GRADER] Solution tuple: %s seconds" % (time.time() - start_time))
		student_docs, actual_output = self.get_student_query_tuple()
		print("[GRADER] Student tuple: %s seconds" % (time.time() - start_time))
		success = False
		if self.student_query_exists:
			flag_1 = True
		...
		print("[GRADER] Answer comparison: %s seconds" % (time.time() - start_time))
