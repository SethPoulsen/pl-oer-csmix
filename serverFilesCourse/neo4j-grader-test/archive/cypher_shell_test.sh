...
echo "[run] ~-= MCW TEST UNDERWAY V2 =-~"
service neo4j start

JOB_DIR='/grade/'
STUDENT_DIR=$JOB_DIR'student/'
# Nothing in serverFilesCourse except for neo4j-grader-test
AG_DIR=$JOB_DIR'serverFilesCourse/neo4j-grader-test/'

# data  serverFilesCourse  shared  student  tests
ls $JOB_DIR
# query.cypher
ls $STUDENT_DIR
# Dataset_Creation.py
# MCW_test.cypher
# (This entire directory. That simplifies things.)
ls $AG_DIR
...
echo "[run] Printing student code"
cat $STUDENT_DIR'query.cypher'
echo -e "\n[run] +++++1"
neo4j-admin set-initial-password neo4j
cypher-shell --version
echo -e "\n[run] +++++2"
cat $AG_DIR'MCW_test.cypher'
echo -e "\n[run] +++++3"
cat $AG_DIR'MCW_test.cypher' | cypher-shell -u neo4j -p neo4j 
echo -e "\n[run] +++++4"
exit 1
